...
//    __disable_irq();
	...
	{
		case DATA_TYPE_8:
            HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, flashAddress , ((uint8_t *)wrBuf)[0]);
		...
		case DATA_TYPE_16:
            HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, flashAddress , ((uint16_t *)wrBuf)[0]);
				for(uint32_t i=0; i<Nsize; i++)
				{
		...
		case DATA_TYPE_32:
            	HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, flashAddress , ((uint32_t *)wrBuf)[0]);
				for(uint32_t i=0; i<Nsize; i++)
				{
